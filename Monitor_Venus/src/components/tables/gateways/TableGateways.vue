...
// Table composables
const { searchText, filteredItems, handleSearch, clearSearch } = useTableSearch(gateways, ['name', 'cs_gateway_id', 'location.name', 'state'])
...
const fetchGateways = async () => {
  ...
  try {
    console.log('🔄 Fetching gateways data...')
    
    // Real API call using await
    const response = await API.get(API.GATEWAY, headers);
