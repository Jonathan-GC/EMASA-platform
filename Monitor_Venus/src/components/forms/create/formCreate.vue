...
// Properly initialize formValues from fields array
const initializeFormValues = (fieldsArray) => {
  ...
  return { ...values, ...additionalData.value };
};
...
function openMap(fieldKey) {
  ...
  const latField = props.fields.find(f => f.key === 'latitude');
  const lngField = props.fields.find(f => f.key === 'longitude');
  
  const latValue = formValues.value.latitude || (latField ? formValues.value.latitude : 0);
  const lngValue = formValues.value.longitude || (lngField ? formValues.value.longitude : 0);
