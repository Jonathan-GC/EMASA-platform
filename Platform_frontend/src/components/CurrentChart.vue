  ...
  <div class="current-dashboard">
    ...
    <div class="device-info" v-if="lastDevice">
      ...
      <div class="info-card">
        ...
        <p><strong>Promedio:</strong> {{ (lastDevice.buffer_stats?.avg_voltage || 0).toFixed(2) }}A</p>
        <p><strong>Rango:</strong> {{ (lastDevice.buffer_stats?.min_voltage || 0).toFixed(1) }}A - {{ (lastDevice.buffer_stats?.max_voltage || 0).toFixed(1) }}A</p>
      </div>
...
const chartOptions = computed<ChartOptions<'line'>>(() => ({
  ...
  plugins: {
    title: {
      display: true,
      text: `Mediciones de Corriente - ${lastDevice.value?.device_name || 'Dispositivo IoT'}`
    },
    legend: {
      display: true
    },
...
const processIncomingData = (data: DeviceData): void => {
  ...
  if (currentValues && Array.isArray(currentValues)) {
    ...
    const newPoints: ChartPoint[] = currentValues.map(sample => ({
    ...
    console.log(`   Rango de corriente: ${Math.min(...newPoints.map(p => p.y)).toFixed(3)}A - ${Math.max(...newPoints.map(p => p.y)).toFixed(3)}A`)
  } else {
...
// Ciclo de vida
onMounted(() => {
  console.log('🚀 Componente CurrentChart montado, iniciando conexión WebSocket...')
  connectWebSocket()
})
